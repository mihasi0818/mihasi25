class UsersController < ApplicationController
  ...
  before_action :set_pagy_and_posts, only: [:index, :show]
  require 'pagy'

  def index
    @pagy_users, @users = pagy(User.order(created_at: :desc), items: 10)
    @pagy_posts, @posts = pagy(Post.order(created_at: :desc), items: 10)
  end
  ...
  def set_pagy_and_posts
    @posts = Post.all # Replace with your own query or assignment
    @pagy, @posts = pagy(@posts, items: 10)
  end
